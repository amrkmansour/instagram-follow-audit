...
The Stripe implementation planner selected a Stripe-only, browser-based, standard checkout integration using full-page Stripe-hosted Checkout. The charge is a one-time $1.99 USD payment for one Instagram follow audit.
...
- The one-time $1.99 USD Price is configured as Worker variable `STRIPE_PRICE_ID`; Price IDs are public configuration, not API secrets.
...
- Replace test values with live Worker secrets and the live $1.99 Price ID, then run one live purchase and refund it from the Dashboard.
